document.addEventListener("DOMContentLoaded", function () {
    ...
    function calculateWeightAndVolume(pm, percentage, setsInput, repsInput) {
        ...
        // Преобразуем введенные данные (например, "5+3+2" → [5, 3, 2])
        const sets = setsInput.split("+").map(Number);
        const reps = repsInput.split("+").map(Number);

        if (sets.includes(NaN) || reps.includes(NaN)) return ["Ошибка", "Ошибка", "Ошибка"];

        let totalReps = 0;
        sets.forEach((set, index) => {
            totalReps += set * reps[index];  // Умножаем количество подходов на количество повторений
        });
    ...
    function updateTable() {
        ...
        if (isNaN(pm)) {
            alert("Пожалуйста, введите правильное значение ПМ.");
            return;
        }
        ...
        for (let week = 1; week <= 4; week++) {
            for (let day = 1; day <= trainingFrequency; day++) {
                ...
                row.innerHTML = `
                    ...
                    <td><input type="text" class="sets" placeholder="Подходы (например, 5+3+2)" size="12"></td>
                    <td><input type="text" class="reps" placeholder="Повторения (например, 5+3+2)" size="12"></td>
                    <td class="kg">-</td>
                    <td class="volume">-</td>
                    <td class="totalReps">-</td>
                `;
    ...
    function addInputListeners() {
        ...
        function recalculate() {
            ...
            rows.forEach(row => {
                ...
                const totalRepsCell = row.querySelector(".totalReps");

                if (percentage && sets && reps) {
                    const [weight, totalReps] = calculateWeightAndVolume(pm, parseFloat(percentage), sets, reps);
                    kgCell.textContent = `${weight} кг`;
                    volumeCell.textContent = `${totalReps} повторений`;
                    totalRepsCell.textContent = `${totalReps} повторений`;
                } else {
                    kgCell.textContent = "-";
                    volumeCell.textContent = "-";
                    totalRepsCell.textContent = "-";
                }
            });
    ...
    function downloadTable() {
        let csv = "Неделя,Тренировка,ПМ (%),Подходы,Повторения,Кг,Объем,Суммарные повторения\n";
